                ...
                LazyColumn(Modifier.fillMaxWidth()) {
                    items(products, key = { it.id }) { product ->
                        product.name?.takeIf { it.isNotBlank() }?.let {
                            Column(Modifier.fillMaxWidth().padding(14.dp).clickable(onClick = {
                                onAction(MainScreenAction.GoToNextScreen(it))
                            }), Arrangement.spacedBy(8.dp), Alignment.Start) {
                                Text(it, style = MaterialTheme.typography.displaySmall)
                                Text(product.category ?: "No category", style = MaterialTheme.typography.bodySmall)
                            }
                        }
                    }
                }
